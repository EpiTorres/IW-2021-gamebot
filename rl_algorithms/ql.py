...
class QLearning():
    ...
    def get_best_action(self, state):
        state_idx = self.state_space.index(state)  # Gets index of the state
        ##################################################################
    ...
    def update(self, state, action, dino, next_state):
        # Uses the state space to get the index of the current
        # state in the q table
        state_idx = self.state_space.index(state)

        # Uses the state space to get the index of the next
        # state in the q table
        next_state_idx = self.state_space.index(next_state)

        ##################################################################
        # Gets the current Q-value in the Q-Table for the
        # current state and action
        old_q = self.qtable[state_idx, action]

        # Gets the highest Q-value in the Q-Table for the next state
        next_q = np.max(self.qtable[next_state_idx])

        # Calculates the new Q-value using the Bellman equation
        new_q = (old_q + self.ql_params['alpha']
                 * (dino.reward + self.ql_params['gamma'] * next_q - old_q))
